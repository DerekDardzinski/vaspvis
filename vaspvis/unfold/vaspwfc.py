...
def save2vesta(
    phi=None, poscar="POSCAR", prefix="wfc", lgam=False, lreal=False, ncol=10
):
    """
    ...
    with open(prefix + "_r.vasp", "w") as out:
        ...
        out.write(
            "\n".join(
                ["".join([fmt % xx for xx in row]) for row in psi_h.real]
            )
        )
    ...
    if not (lgam or lreal):
        with open(prefix + "_i.vasp", "w") as out:
            ...
            out.write(
                "\n".join(
                    ["".join([fmt % xx for xx in row]) for row in psi_h.imag]
                )
            )
...
class vaspwfc(object):
    ...
    def __init__(
        ...
        assert (
            self._gam_half == "x" or self._gam_half == "z"
    ...
    def readWFHeader(self):
        ...
        self._recl, self._nspin, self._rtag = np.array(
            np.fromfile(self._wfc, dtype=np.float64, count=3), dtype=np.int64
        )
    ...
    def readWFBand(self):
        ...
        for ii in range(self._nspin):
            for jj in range(self._nkpts):
                ...
                dump = np.fromfile(
                    self._wfc, dtype=np.float64, count=4 + 3 * self._nbands
                )
                if ii == 0:
    ...
    def gvectors(self, ikpt=1, force_Gamma=False, check_consistency=True):
        ...
        gz, gy, gx = np.array(np.meshgrid(fz, fy, fx, indexing="ij")).reshape(
            (3, -1)
        )
            ...
            if Gvec.shape[0] != self._nplws[ikpt - 1]:
                if Gvec.shape[0] * 2 == self._nplws[ikpt - 1]:
                    ...
                    """
                        % (
                            Gvec.shape[0],
                            self._nplws[ikpt - 1] // 2
                            if self._lsoc
                            else self._nplws[ikpt - 1],
                            np.prod(self._ngrid),
                        )
                    )
    ...
    def save2vesta(
        self, phi=None, lreal=False, poscar="POSCAR", prefix="wfc", ncol=10
    ):
        """
        Save the real space pseudo-wavefunction as vesta format.
        """
                ...
                if self._gam_half == "z":
                    for ii in range(ngrid[0]):
                        for jj in range(ngrid[1]):
                            fx = (
                                ii if ii < ngrid[0] // 2 + 1 else ii - ngrid[0]
                            )
                            fy = (
                                jj if jj < ngrid[1] // 2 + 1 else jj - ngrid[1]
                            )
                    ...
                    for jj in range(ngrid[1]):
                        for kk in range(ngrid[2]):
                            ...
                            fz = (
                                kk if kk < ngrid[2] // 2 + 1 else kk - ngrid[2]
                            )
                    ...
                    tmp = (
                        np.fft.irfftn(phi_k, s=(ngrid[2], ngrid[1], ngrid[0]))
                        * normFac
                    )
    ...
    def poisson(
        self, rho=None, iband=1, ikpt=1, ispin=1, ngrid=None, norm=False
    ):
        """
        ...
        if rho is not None:
            ...
            if self._lsoc:
                ...
                phi_spinor = self.wfc_r(
                    iband=iband, ikpt=ikpt, ispin=ispin, ngrid=ngrid, norm=norm
                )
    ...
    def TransitionDipoleMoment(self, ks_i, ks_j, norm=True, realspace=False):
        """
        ...
        if realspace:
            ...
            tdm = np.array(
                [np.sum(pij * Rx), np.sum(pij * Ry), np.sum(pij * Rz)]
            )
            ovlap = pij.sum()
        else:
        ...
        for ispin in range(self._nspin):
            for ikpt in range(self._nkpts):
                for iband in range(self._nbands):
                    phi_j = self.wfc_r(
                        ispin + 1, ikpt + 1, iband + 1, norm=norm
                    )
                    ...
                    self.ipr[ispin, ikpt, iband, 1] = self._bands[
                        ispin, ikpt, iband
                    ]
    ...
    def elf(self, kptw, ngrid=None, warn=True):
        ...
        assert kptw.shape == (
            self._nkpts,
        ), "K-point weights must be provided \
        ...
        for ispin in range(self._nspin):
            ...
            for ikpt in range(self._nkpts):
                ...
                for iband in range(self._nbands):
                    ...
                    weight = (
                        rspin * kptw[ikpt] * self._occs[ispin, ikpt, iband]
                    )
            ...
            Cf = 3.0 / 5 * (3.0 * np.pi**2) ** (2.0 / 3)
            Dh = np.where(rho > 0.0, Cf * rho ** (5.0 / 3), 0.0)
            eps = 1e-8 / HSQDTM
            Dh[Dh < eps] = eps
            # D0 = T + TCORR - TBOS
            D0 = tau + 0.5 * lap_rho_r - 0.25 * grad_rho_sq / rho

            ElectronLocalizationFunction.append(1.0 / (1.0 + (D0 / Dh) ** 2))
